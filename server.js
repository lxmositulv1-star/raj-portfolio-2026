...
// API 2: Add Reseller (Fixing Route for 404/Unexpected token error)
app.post('/api/add-reseller', async (req, res) => {
  try {
    const { username, credits } = req.body;
    if (!username || credits === undefined) {
      return res.status(400).json({ success: false, message: 'KEY and credits required!' });
    }
    ...
    if (reseller) {
      ...
      reseller = new Reseller({ username, credits: Number(credits) });
      await reseller.save();
    }

    res.json({ success: true, message: 'Reseller updated successfully!', data: reseller });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
});

// API 3: Reseller Generate Key
app.post('/api/reseller-generate', async (req, res) => {
  try {
    ...
    if (!reseller || reseller.credits <= 0) {
      return res.status(403).json({ success: false, message: 'Invalid KEY or No Credits Left!' });
    }
